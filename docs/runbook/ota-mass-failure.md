...
OTA 推送告警风暴
  ...
  │
  ├─ 立即停止后续 OTA 调度
  │    docker exec admin curl -s -X POST http://admin:8082/internal/ota/abort-all
  │
  └─ 评估是否影响充电业务
...
docker exec mysql mysql -uroot -p$MYSQL_ROOT_PASSWORD \
  -e "UPDATE gateway_db.device SET status='fault', reason='ota_failure_$(date +%Y%m%d)' WHERE firmware_version='新版本';"
...
- **强制签名校验**:`ota_package.signature` 用厂商私钥签名 + SHA-256,设备端严格验证(已实现,见 `技术规格.md` § 6.6)
- **升级窗口显式标注**:在 admin PC 后台发 OTA 时必须确认"凌晨 02:00-04:00 维护窗口"+ 短信通知到客户运维
- **预发布灰度**:技术规格 § 6.6 说明本期不做灰度,**但客户在自有小范围预发布后**,需要客户在系统外记录"灰度结果" → 上线前必须看到这份记录
- **失败率告警**:升级窗口内如果失败率 > 5% → 自动 abort,告警推送给客户管理员(代码动手时实现)
